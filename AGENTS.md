...
15. **首音门控按阶段而非固定帧数**：Native 的 60 FPS 截图循环不得使用固定 200 帧冻结期。单人/校准/挑战从开始过渡阶段进入，协力从生命条已确认阶段进入；二者只允许使用不同的连续稳定窗口，不得引入不同的歌曲时间偏移。稳定期遇到显著变化必须重置，冻结后保留 500 ms 前奏残留宽限，并在结果报告中保存等待、忽略和触发证据。
