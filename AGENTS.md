...
16. **最终封面确认谱面，首音只负责定时**：单人、校准、挑战和协力在完整演奏场前都必须观察最终歌曲信息页，使用居中封面复核本轮已解析的本地谱面；封面不得作为歌曲时钟起点。共享封面继续结合准备页等级与本地标题 OCR 消歧，等级冲突硬拒绝。生命保护关闭时禁止继续构造数值 `LifeDetector`，但必须保留演奏场启动门控和约 5 Hz 的终态监控；协力不得先等到演奏场出现再启动会话。
