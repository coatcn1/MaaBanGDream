...
| 实时演奏 | 进行中 | 连续任务固定使用排练模式，无需 Profile；支持当前/随机曲目、五档难度、1–99 次及本机整曲调试录制；正式演奏暂不开放 |
| Profile 系统 | 进行中 | Profile v1 草稿、原子验收、环境签名和最新已验收 Profile 解析已接入；Easy Profile 已由用户真机验收并在本机激活 |
| 完整歌曲与结算反馈 | 进行中 | 已实现演出结束状态机、最长 300 秒门禁、触点清理和本机结算截图；待真机采集后接入 FAST/SLOW 解析 |
| 挑战演出 | 未开始 | 在自动演出基础上实现 |
...
2. Profile 校准接入
   - 已完成 Easy 草稿生成、用户验收激活和 Profile 驱动排练入口；正在采集完整歌曲结算画面，随后用 FAST/SLOW 反馈微调时序偏移。
3. 挑战演出
