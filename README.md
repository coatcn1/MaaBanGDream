...
| 最终封面门控 | 单人、校准、挑战和协力均观察最终歌曲信息页；协力漏检短黑场时，连续两帧且与本局身份一致的封面可直接证明正常转场。一般难度无可信谱面时在触控前整局回退 Legacy；Special 因左右 Directional 无法由纯视觉可靠恢复，会明确停止而不盲打 |
...
Special 的难度策略不会静默冒充：单人实时、挑战和自动演出在当前歌曲没有 Special 时明确失败；协力因歌曲由房间决定，可回退到 Expert，并在日志与结果中同时保留请求难度和实际难度。Special 实时演奏必须命中可信本地谱面；Legacy 与 Native 都按谱面恢复 Left / Right，`width=1..7` 作为轨道跨度证据保留。雷电 Native 单人真机已覆盖普通 Special 与包含 Left/Right、`width=1..3` 的 Directional 谱面；协力回退和开演门控仍应在真实房间中单独验收。
