...
- 新增单轮 `LiveRunContext` 与无额外截图的 `song-phash-v1` 歌曲指纹；正式结果、校准报告和 Debug Recorder 通过同一 `run_id`/`song_id` 关联，所有失败终态输出结构化结果。
...
- 新增隔离的“视觉设置评估（实验）”模式。实验可复用已验收 Profile 的几何与偏移，但不得接受、覆盖 Profile 或计入正式成功率。
...
- 只截图观察与离线重放增加结构化截图基准和长帧/丢帧故障注入，为独立的截图卡顿修复阶段提供证据。
