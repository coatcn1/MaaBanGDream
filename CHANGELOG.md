...
- 当前待验收项：共享封面歌曲的 Normal 真机复验，以及三排练一正式校准的完整断点续跑。

## 2026-09-01（v1.0.1 开发中）

- 修复校准演出生命耗尽后的恢复死循环：生命保护已经确认终止时不再在死亡弹窗与退出确认之间反复 BACK，而是先重启游戏，再进入有界登录恢复；普通演出结算仍沿用原返回主页流程。
- 登录页右下角菜单模板阈值按便携版实机命中分从 0.90 调整为 0.84；谱面预测器允许密集段最多 9 个同方向伪残差，第 10 个连续失配才安全回退，同时保留 8 个一致样本的早期锁相纠偏。确认谱面后的两点和多点滑条改为连续路径驱动，并丢弃同一帧会与谱面轨迹冲突的视觉跟随 MOVE。
- 修复便携启动器重复启动兼容性：Windows PowerShell 生成的运行配置统一写为无 BOM UTF-8，运行时检查同时兼容旧启动器已经生成的 BOM 文件，避免首次启动后第二次卡在 JSON 自检。
